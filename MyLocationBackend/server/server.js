...
server.post('/signup',(req,res)=>{
   ...
   fs.readFile('../db/users.json', (err,data) => {
        ...
        let duplicateUser = utilityFunctions.checkDuplicateUser(userData.users,email) 
...
server.post('/login', (req,res) => {
  ...
  fs.readFile('../db/users.json',(err,data) => {
        ...
        let userData = JSON.parse(data.toString())
        let checkUserExistsOrNot = utilityFunctions.checkDuplicateUser(userData.users,email)
        if(!checkUserExistsOrNot) {
            let checkPassword = utilityFunctions.checkPassword(userData.users,password)
            if(checkPassword) {
                ...
                let token = utilityFunctions.createToken({email,password,userid,lastname})
                let message = "user logged in"
                res.status(200).json({token,message,userid,lastname,registrationType})
                return
            } else {
